#!/usr/bin/env Rscript
# SN label-transfer pathway analysis stage (R):
# - Reads Python handoff files from SN label transfer pipeline
...
if (!requireNamespace("fgsea", quietly = TRUE)) {
    stop("Package 'fgsea' is required for SN label-transfer GSEA. Install it before running this script.")
}
...
coverage_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_coverage_summary.csv")
overlap_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_pairwise_gene_overlap.csv")
shared_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_shared_universe.csv")
pb_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_pseudobulk_matrix.csv")
design_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_sample_design.csv")
diag_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_sample_diagnostics.csv")
transfer_diag_path <- file.path(output_dir, "myeloid_sn_label_transfer_py_transfer_diagnostics.csv")

# These are produced by myeloid_sn_label_transfer_prepare.py in the same folder.
...
transfer_diag_dt <- if (file.exists(transfer_diag_path)) fread(transfer_diag_path) else data.table()
...
n_used_samples <- uniqueN(design_dt$sample)
analysis_mode <- if (n_used_samples >= 2L) "cohort_limma" else "single_sample_exploratory"

if (n_used_samples >= 2L) {
    # Match spatial-imputation branch: within-sample Myeloid 2 vs Myeloid 1 contrast.
    ...
    m1_cols <- design_dt[group == "Myeloid 1", column_id]
    m2_cols <- design_dt[group == "Myeloid 2", column_id]
    if (length(m1_cols) != 1L || length(m2_cols) != 1L) {
        stop("Single-sample exploratory mode requires exactly one Myeloid 1 and one Myeloid 2 pseudobulk column.")
    }
    logfc_vec <- pb_mat[, m2_cols] - pb_mat[, m1_cols]
    de_dt <- data.table(
        gene = toupper(rownames(pb_mat)),
        logFC_m2_vs_m1 = as.numeric(logfc_vec),
        t_stat = as.numeric(logfc_vec),
        pvalue = NA_real_,
        fdr = NA_real_
    )
    de_dt <- unique(de_dt, by = "gene")
...
if (nrow(transfer_diag_dt) > 0L) {
    sample_diag_out <- merge(sample_diag_out, transfer_diag_dt, by = "sample", all.x = TRUE)
}

fwrite(coverage_out, file.path(output_dir, "myeloid_sn_label_transfer_coverage_summary.csv"))
fwrite(pairwise_out, file.path(output_dir, "myeloid_sn_label_transfer_pairwise_gene_overlap.csv"))
fwrite(shared_out, file.path(output_dir, "myeloid_sn_label_transfer_shared_universe.csv"))
fwrite(limma_out, file.path(output_dir, "myeloid_sn_label_transfer_limma_de.csv"))
fwrite(geneset_out, file.path(output_dir, "myeloid_sn_label_transfer_gene_sets_filtered.csv"))
fwrite(gsea_out, file.path(output_dir, "myeloid_sn_label_transfer_gsea_results.csv"))
fwrite(sample_diag_out, file.path(output_dir, "myeloid_sn_label_transfer_sample_diagnostics.csv"))

metadata_path <- file.path(output_dir, "myeloid_sn_label_transfer_metadata.txt")
...
metadata_lines <- c(
    ...
    paste0("n_samples_used: ", n_used_samples),
    ...
    paste0("transfer_mode: weighted_knn_in_latent_umap"),
    paste0("transfer_k: 25"),
    paste0("sn_subset_rule: annot_v1 exact in {Monocytes, Microglia, Macrophage}"),
...
cat("Saved SN label-transfer outputs to:", output_dir, "\n")
