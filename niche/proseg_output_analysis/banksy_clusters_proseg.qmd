...
banksy_embeddings_seurat <- RunPCA(
    banksy_embeddings_seurat, assay = "SCT", verbose = FALSE,
    rev.pca = TRUE, reduction.name = "pca.rev",
    reduction.key="PCR_", npcs = 50
)
E <- banksy_embeddings_seurat@reductions$pca.rev@feature.loadings
...
ggplot(histology_cell_type_enrichment) +
    geom_point(aes(
        x = as.factor(as.numeric(cell_type)), 
...
#             `Fine Cell Type UMAP`
#         )],
#         on = c('cluster' = 'cluster')
#     ][
#         , .(proportion = as.numeric(.N)), 
#         .(sample_id, histology, `Fine Cell Type UMAP`)
#     ][, proportion := proportion/sum(proportion), .(sample_id, histology)],
#     sample_id + histology ~ `Fine Cell Type UMAP`,
#     value.var = 'proportion', fill = 0
# ),padj_method = 'BH')
