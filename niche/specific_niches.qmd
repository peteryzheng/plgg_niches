...
# Look at specific niches in specific histologies
...
ggplot(
    graph_dt[
        ...
        `Cluster ID` %in% c(4,5,10,13) & 
        grepl('pilocytic',histology)
    ], 
    aes(x=sdimy, y=sdimx, color=`Cluster ID`)
) +
    facet_wrap(
        sample_id ~ `Cluster ID`, 
        nrow = 4, scale = 'free_x'
    ) +
    # subset cells
    # geom_point(size = 0.3, alpha = 0.5) + 
    ...
    theme(
        ...
        color = guide_legend(override.aes = list(size=5, alpha = 1), nrow = 1, byrow = TRUE)
    ) + labs(
        title = paste0('PA Niches'),
        x = '',y = ''
    ) +
    coord_flip()
```
...
ggplot(
    graph_dt[
        `Cluster Type` == niche_clust_name & 
        `Cluster ID` %in% c(4,10,11) & 
        grepl('cerebellar',histology)
    ], 
    aes(x=sdimy, y=sdimx, color=`Cluster ID`)
) +
    facet_wrap(
...
ggplot(
    graph_dt[
        `Cluster Type` == niche_clust_name & 
        `Cluster ID` %in% c(7, 14, 17, 20, 22) & 
        grepl('ganglio',histology)
    ], 
    aes(x=sdimy, y=sdimx, color=`Cluster ID`)
) +
    facet_wrap(
...
ggplot(
    graph_dt[
        `Cluster Type` == niche_clust_name & 
        `Cluster ID` %in% c(3, 8, 11, 15, 16) & 
        grepl('central',histology)
    ], 
    aes(x=sdimy, y=sdimx, color=`Cluster ID`)
) +
    facet_wrap(
